...
class MultiQuerySearchBackend:
    ...
    def search(self, query: str, *, top_k: int) -> list[SearchHit]:
        ...
        merged = merge_ranked_hits(all_hits, top_k=top_k)
        self.last_debug = {
...
def merge_ranked_hits(hits: list[SearchHit], *, top_k: int) -> list[SearchHit]:
    if top_k <= 0:
        return []
...
def _rank_key(hit: SearchHit) -> tuple[int, int, int]:
    return (_official_domain_boost(hit.url), _doc_path_boost(hit.url), -int(hit.rank))


def _official_domain_boost(url: str) -> int:
